      ...
      <main className="flex-grow bg-background py-16 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-center space-x-10 mb-8">
            ...
            <button className="text-xl font-semibold text-status-success-text border-b-4 border-emerald-600">
              Rentals
            </button>
          </div>
          ...
          <section className="animate-border-glow rounded-xl p-8 shadow-md">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {rentals.map((rental) => (
                <Link
                  key={rental.bike_id}
                  href={`/products/${rental.bike_id}?rental=true`}
                >
                  <div className="flex flex-col items-center bg-background rounded-xl border-2 border-emerald-200 p-4 transition-all duration-300 hover:-translate-y-1 hover-border-glow">
                    ...
                    <div className="text-center space-y-2 w-full bg-slate-100 p-3 rounded-lg border border-slate-200">
                      <h3 className="text-lg font-semibold text-status-success-text hover:text-text-link transition-colors">
                        {rental.name}
                      </h3>
